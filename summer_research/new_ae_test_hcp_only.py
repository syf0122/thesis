...
'''
    Load Trained Models
'''
autoencoder_360 = load_model('/data_qnap/yifeis/NAS/ae_updated/model_hcp/autoencoder_360.hdf5', compile=False)
encoder_360 = load_model('/data_qnap/yifeis/NAS/ae_updated/model_hcp/encoder_360.hdf5', compile=False)
decoder_360 = load_model('/data_qnap/yifeis/NAS/ae_updated/model_hcp/decoder_360.hdf5', compile=False)

autoencoder_379 = load_model('/data_qnap/yifeis/NAS/ae_updated/model_hcp/autoencoder_379.hdf5', compile=False)
encoder_379 = load_model('/data_qnap/yifeis/NAS/ae_updated/model_hcp/encoder_379.hdf5', compile=False)
decoder_379 = load_model('/data_qnap/yifeis/NAS/ae_updated/model_hcp/decoder_379.hdf5', compile=False)
...
if len(sys.argv) == 2:
    ...
    for sub in test_subjects[:50]:
        ...
        for i in range(len(test_re)):
            ...
            mtx_recon_df.to_csv("/data_qnap/yifeis/NAS/ae_updated/results/hcp_only/recon/"+group+"_"+str(sub)+"_"+run+"_2mm_"+prepro+"_recon_df.csv")
            # save latent data
            mtx_latent_df = pd.DataFrame(data=test_latent_re[i]) # (149, 45)
            mtx_latent_df.to_csv("/data_qnap/yifeis/NAS/ae_updated/results/hcp_only/latent/"+group+"_"+str(sub)+"_"+run+"_2mm_"+prepro+"_latent_df.csv")
        ...
        for i in range(len(test_re)):
            ...
            # save reconstructed data
            mtx_recon_df = pd.DataFrame(data=test_re[i]) # (149, 360)
            mtx_recon_df.to_csv("/data_qnap/yifeis/NAS/ae_updated/results/hcp_only/recon/"+group+"_"+str(sub)+"_"+run+"_4mm_"+prepro+"_recon_df.csv")
            # save latent data
            mtx_latent_df = pd.DataFrame(data=test_latent_re[i]) # (149, 45)
            mtx_latent_df.to_csv("/data_qnap/yifeis/NAS/ae_updated/results/hcp_only/latent/"+group+"_"+str(sub)+"_"+run+"_4mm_"+prepro+"_latent_df.csv")
        ...
        for i in range(len(test_re)):
            ...
            mtx_recon_df.to_csv("/data_qnap/yifeis/NAS/ae_updated/results/hcp_only/recon/"+group+"_"+str(sub)+"_"+run+"_surf_"+prepro+"_recon_df.csv")
            # save latent data
            mtx_latent_df = pd.DataFrame(data=test_latent_re[i]) # (149, 40)
            mtx_latent_df.to_csv("/data_qnap/yifeis/NAS/ae_updated/results/hcp_only/latent/"+group+"_"+str(sub)+"_"+run+"_surf_"+prepro+"_latent_df.csv")
    ...
    for sub in test_subjects[:50]:
        ...
        mse_data_sub.to_csv("/data_qnap/yifeis/NAS/ae_updated/results/hcp_only/MSE/"+group+"_"+str(sub)+"_2mm_"+prepro+"_regional_MSE.csv")
        ...
        mse_data_sub = pd.DataFrame(data=mse_data_sub, index=rows, columns=cols) # (2, 360)
        mse_data_sub.to_csv("/data_qnap/yifeis/NAS/ae_updated/results/hcp_only/MSE/"+group+"_"+str(sub)+"_4mm_"+prepro+"_regional_MSE.csv")
        ...
        mse_data_sub.to_csv("/data_qnap/yifeis/NAS/ae_updated/results/hcp_only/MSE/"+group+"_"+str(sub)+"_surf_"+prepro+"_regional_MSE.csv")
    ...
    for sub in test_subjects[:50]:
        ...
        for i in range(len(mse_sub)):
            ...
            # save image of mse
            nlp.view_surf(hcp.mesh.inflated,hcp.cortex_data(hcp.unparcellate(mse, hcp.mmp)), bg_map=hcp.mesh.sulc, symmetric_cmap = False, vmax = 0.01, vmin = 0).save_as_html('/data_qnap/yifeis/NAS/ae_updated/results/hcp_only/plots/'+group+"_"+str(sub)+"_"+run+"_2mm_"+prepro+'_mse.html')
        ...
        for i in range(len(mse_sub)):
            ...
            mse = np.concatenate((mse, pad))
            # save image of mse
            nlp.view_surf(hcp.mesh.inflated,hcp.cortex_data(hcp.unparcellate(mse, hcp.mmp)), bg_map=hcp.mesh.sulc, symmetric_cmap = False, vmax = 0.01, vmin = 0).save_as_html('/data_qnap/yifeis/NAS/ae_updated/results/hcp_only/plots/'+group+"_"+str(sub)+"_"+run+"_4mm_"+prepro+'_mse.html')
        ...
        for i in range(len(mse_sub)):
            ...
            run = test_ses[i]
            # save image of mse
            nlp.view_surf(hcp.mesh.inflated,hcp.cortex_data(hcp.unparcellate(mse, hcp.mmp)), bg_map=hcp.mesh.sulc, symmetric_cmap = False, vmax = 0.01, vmin = 0).save_as_html('/data_qnap/yifeis/NAS/ae_updated/results/hcp_only/plots/'+group+"_"+str(sub)+"_"+run+"_surf_"+prepro+'_mse.html')
else:
    ...
    for sub in hcp_subjects:
        ...
        for i in range(len(test_re)):
            ...
            mtx_recon_df.to_csv("/data_qnap/yifeis/NAS/ae_updated/results/hcp_only/recon/HCP_"+str(sub)+"_"+run+"_360_recon_df.csv")
            # save latent data
            mtx_latent_df = pd.DataFrame(data=test_latent_re[i]) # (900, 45)
            mtx_latent_df.to_csv("/data_qnap/yifeis/NAS/ae_updated/results/hcp_only/latent/HCP_"+str(sub)+"_"+run+"_360_latent_df.csv")
        ...
        for i in range(len(test_re)):
            ...
            mtx_recon_df.to_csv("/data_qnap/yifeis/NAS/ae_updated/results/hcp_only/recon/HCP_"+str(sub)+"_"+run+"_379_recon_df.csv")
            # save latent data
            mtx_latent_df = pd.DataFrame(data=test_latent_re[i]) # (900, 40)
            mtx_latent_df.to_csv("/data_qnap/yifeis/NAS/ae_updated/results/hcp_only/latent/HCP_"+str(sub)+"_"+run+"_379_latent_df.csv")
    ...
    for sub in hcp_subjects:
        ...
        mse_data_sub = pd.DataFrame(data=mse_data_sub, index=rows, columns=cols) # (2, 360)
        mse_data_sub.to_csv("/data_qnap/yifeis/NAS/ae_updated/results/hcp_only/MSE/HCP_"+str(sub)+"_360_regional_MSE.csv")
        ...
        rows = hcp_session[sub]
        mse_data_sub = pd.DataFrame(data=mse_data_sub, index=rows, columns=cols) # (2, 379)
        mse_data_sub.to_csv("/data_qnap/yifeis/NAS/ae_updated/results/hcp_only/MSE/HCP_"+str(sub)+"_379_regional_MSE.csv")
    ...
    for sub in hcp_subjects:
        ...
        for i in range(len(mse_sub)):
            ...
            mse = np.concatenate((mse, pad))
            # save image of mse
            nlp.view_surf(hcp.mesh.inflated,hcp.cortex_data(hcp.unparcellate(mse, hcp.mmp)), bg_map=hcp.mesh.sulc, symmetric_cmap = False, vmax = 0.01, vmin = 0).save_as_html('/data_qnap/yifeis/NAS/ae_updated/results/hcp_only/plots/HCP_'+str(sub)+"_"+run+'_360_mse.html')
        ...
        for i in range(len(mse_sub)):
            ...
            run = test_ses[i]
            # save image of mse
            nlp.view_surf(hcp.mesh.inflated,hcp.cortex_data(hcp.unparcellate(mse, hcp.mmp)), bg_map=hcp.mesh.sulc, symmetric_cmap = False, vmax = 0.01, vmin = 0).save_as_html('/data_qnap/yifeis/NAS/ae_updated/results/hcp_only/plots/HCP_'+str(sub)+"_"+run+'_379_mse.html')
